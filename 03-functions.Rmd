...
Speaking of lists, `map_*` functions are known to take in list inputs, and can create list-columns depending on the specific `map_*` function employed.

```{r}
tibble(x = c(3, 7, 2)) %>% 
  mutate(sq_root = map(x, sqrt))
```
...
Only... what exactly is a list-column? Our next section will give you a quick refresher on lists, then dive into how lists, list-columns, and `map_*` functions work with each other.
...
tmp <- nhanes %>%
  ...
  drop_na() %>% 
  group_by(gender) %>% 
  summarize(height_quintiles = list(quantile(height)),
            .groups = "drop")

tmp
```

Take a look at the values for `height_quintiles`:
