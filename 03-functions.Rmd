...
<!-- * This will now be week 3. Need to focus more on functions which actually do something useful, something that teaches concepts which will be valuable later. Should also delete material that is not that useful. Think about what aspects of functions are helpful for using Shiny and for final projects in general. -->

<!-- Key concept to focus on is the idea of a distribution and draws therefrom. This connects to chapter 2, where we introduced distributions and to chapter 5, where we explain probability.  -->

<!-- Key question: Do we explain standard error here? I think No. But we should explain everything short of standard error. In particular, we should show various ways to measure center and variability. These are cool and important concepts. Is "center" the mean or the median or the trimmed mean or . . . Is "variability" the standard deviation, the mean absolute deviation, the standard deviation of the median absolute deviation or . . . There is nothing explicitly "statistical" here. We are just trying to connect English words to specific formulas. -->

<!-- This allows us to get into the "objective function" and even the prediction game. Indeed, the key demonstration at the end, instead of the crooked casino, might be to build a simple prediction game and see who wins. Perhaps showing that median is better than mean if the objective function is the mean absolute deviation. -->

<!-- These below are older comments which might not be wrong . . . -->
...
In this section, we will be bringing together `map_*` functions, list-columns, and function writing. We'll also be introducing some important conditional functions (`ifelse()`, `any()`, `all()`, and `case_when()`). 
