...
```{r, echo = FALSE, eval = TRUE, include = FALSE, cache = TRUE, warning = FALSE, cache.lazy = FALSE}
...
# Census: note dimensions `r prettyNum(nrow(census1),big.mark=",",scientific=FALSE)` by `r ncol(census1)`
...
* This data has `r prettyNum(nrow(census1), big.mark= ",", scientific=FALSE)` rows (big data!), so you may need to take steps not to crash your machine such as sampling the data initially or by working on the FAS cloud system.
...
By this point you may well have noticed that there is much more to the IPUMS offerings than just U.S. population stats. Below you can see what an extract from the M.E.P.S. (Medical Expenditure Panel Survey) or International repositories could look like. Always bear in mind that with an individual access account, you can create almost any configuration you need. While the names and certainly the available interpretations are different, the process by which we import this new data frame and transform any weird variables into our preferred format is largely the same!


```{r, echo = FALSE, cache = TRUE, message = FALSE, include = FALSE, cache.lazy = FALSE}

library(tidyverse)

# load in MEPS example data

ddi_meps <- read_ipums_ddi("IPUMS/extracts/meps/meps_00003.xml")
meps1 <- read_ipums_micro(ddi_meps)

```

MEPS: note dimensions `r prettyNum(nrow(meps1),big.mark=",",scientific=FALSE)` by `r ncol(meps1)`

<!-- pending request for bigger MEPS data with extended var list as discussed -->

```{r show_meps, warning = FALSE}
...
int_data <- read_ipums_micro(ddi_intntl)

```

International: note dimensions `r prettyNum(nrow(int_data), big.mark="," , scientific=FALSE)` by `r ncol(int_data)`
...
## What can you I with all of this? 
...
Here are some examples using the IPUMS data we have been looking at so far: 
