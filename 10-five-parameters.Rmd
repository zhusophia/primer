...
Having created models with one parameter in Chapter \@ref(one-parameter), two parameters in Chapter \@ref(two-parameters), three parameters in Chapter \@ref(three-parameters), and five parameters in Chapter \@ref(five-parameters), you are now ready to make the jump to $N$ parameters. 
...
In this example, we want to center the value for `election_age`, the independent variable. First, we must pick the value that we will center by. Here, we will use the mean of `election_age`, which is about `r round(mean(ch9$election_age), 0)` years old.  Once we find this number, we will subtract it from every `election_age` value. We have, thereby, changed the meaning of `election_age`. It now means the number of years a candidate has been alive, as of Election Day, relative to the average years alive of all candidates on their respective election days.

```{r}
ch9$election_age <- ch9$election_age - mean(ch9$election_age)
```



```{r, cache = TRUE}
fit_gov_1_centered <- stan_glm(data = ch9,
                      formula = lived_after ~ election_age,
                      refresh = 0,
                      seed = 11)
...
* $\beta_2$ is, for the entire population, the average difference in $lived\_after_i$ between two individuals, one of whom has an $election\_age_i$ value of 1 greater than the other. 
...
Now take a look at the coefficient for $election\_age_i$, $\beta_2$. The median of the posterior, `r round(coef(fit_gov_3)["election_age"], 1)`, represents the slope of the model. When comparing two candates who differ by one year in `election_age`, we expect that they will differ by `r round(coef(fit_gov_3)["election_age"], 1)` years in `lived_after`. It makes sense that this value is negative. Think about it: the more years a candidate has lived, the fewer years the candidate has left to live. So, for every extra year a candidate is alive before an election, their lifespan after the election will be `r abs(round(coef(fit_gov_3)["election_age"], 1))` years lower, on average.
...
Let's now take a look at a posterior distribution for $\beta_2$, the coefficient of $election\_age_i$. 
...
Let's create another model. This time, however, the numeric outcome variable of lived_after as a function of the two explanatory variables we used above, `election_age` and `sex`, and of their interaction.
...
* $male_i$ is still one of our explanatory variables. If we are predicting the amount of years a male candidate lives after the election, this will be 1. When we are making this prediction for female candidate, this will be 0. $election\_age_i$ is still our other explanatory variable, representing the number of years a candidate has lived before the election, relative to the average value for all candidates. 
...
* $\beta_2$ is the coefficient of $election\_age_i$. It it just the slope for women. It is the average difference in $lived\_after_i$ between two women, one of whom has an $election\_age_i$ value of 1 greater than the other. In our last example,  $\beta_2$ was the slope for the whole population. Now we are different slopes for different genders. 
...
The intercept has increased. $\beta_0$ is around `r round(coef(fit_gov_4)["(Intercept)"], 0)`. This is the intercept for females. It still means the average number of years lived after the election for women is `r round(coef(fit_gov_4)["(Intercept)"], 0)` or so. Our `sexMale` coefficient, $\beta_1$, refers to the value that must be added to the intercept in order to get the average for males. When calculated, the result is `r round(coef(fit_gov_4)["(Intercept)"] + coef(fit_gov_4)["sexMale"], 0)`. Keep in mind, however, that these values only apply if $election\_age_i = 0$, if, that is, candidate $i$ is around 52 years old. 

The coefficient for $election\_age_i$, $\beta_2$, is `r round(coef(fit_gov_4)["election_age"], 1)`. What does this mean? It is the slope for females. So, when comparing two female candidates who differ by one year in age, we expect that the older candidate will live `r abs(round(coef(fit_gov_4)["election_age"], 1))` years less. Now direct your attention below at the coefficient of `sexMale:election_age`, $\beta_3$, which is `r round(coef(fit_gov_4)["sexMale:election_age"], 1)`. This is the value that must be added to the coefficient of $election\_age_i$ (recall $\beta_2 + \beta_3$) in order to find the slope for males. When the two are added together, this value, or slope, is about `r round(coef(fit_gov_4)["election_age"] + coef(fit_gov_4)["sexMale:election_age"], 1)`. When comparing two male candidates who differ in age by one year, we expect the older candidate to live about `r abs(round(coef(fit_gov_4)["election_age"] + coef(fit_gov_4)["sexMale:election_age"], 1))` years less. 

*Key point*: The interpretation of the intercepts only apply to candidates for whom $election\_age_i = 0$. Candidates who are not 52 years-old will have different expected number of years to live. The interpretation of the slope applies to everyone. In other words, the relationship between $lived\_after_i$ and $election\_age_i$ is the same, regardless of your gender or how old you are.
...
$$ lived\_after_i =  \beta_0 + \beta_1 male_i + \beta_2 election\_age_i + \\ \beta_3 male_i *  election\_age_i + \epsilon_i$$
In both cases, $\beta_0$ has the same meaning. It is the average number of years lived after the election for women, who on the day of election, have been alive the average number of years of all candidates. Plug in 0 for $male_i$ and 0 for $election\_age_i$ and both equations simplify to:
...
The (Intercept) refers to the average number of years lived after the election for women from Alabama, who on the day of election, have been alive the average number of years of all female candidates! That's a mouthful. It really is not different than what we have been doing all along, however. You can see here that (Intercept) value is around `r round(coef(fit_gov_5)["(Intercept)"], 0)` years. 


However, we have only talked about women. What about for the men? Well, if you look at the bottom of the list, you will see the `sexMale` value of around `r round(coef(fit_gov_5)["sexMale"], 0)`. When you wish to estimate `lived_after` for a specific state, you must add this value to the intercept, and then also add the intercept of the desired state. 

$\beta_{51}$, our coefficient value for $election\_age_i$, is `r round(coef(fit_gov_5)["election_age"], 2)`. When comparing female candidates who differ by one year in their current ages, there is not much of a difference in how many years we expect them to live after the election.

<!-- DK: Conflict between $election\_age_i$ and `sexMale:election_age`. Which is better? Should be consistent. -->

$\beta_{51} + \beta_{52}$, our coefficients for $election\_age_i$ and `sexMale:election_age`, results in the value of around `r round(coef(fit_gov_5)["election_age"] + coef(fit_gov_5)["sexMale:election_age"], 1)`. This value is our slope for men.
...
fit_gov_5 %>% 
  ...
  mutate(WA = `(Intercept)` + stateWashington + sexMale + 
           (`sexMale:election_age` + election_age) * 10 * 365) %>% 
  ...
  pivot_longer(cols = WA:SD, 
               names_to = "Parameter",
               values_to = "years") %>% 
  ggplot(aes(years, fill = Parameter)) +
    geom_histogram(aes(y = after_stat(count/sum(count))),
                   alpha = 0.5, 
                   bins = 100, 
                   position = "identity") +
    labs(title = "Posterior Probability Distribution",
         ...
         x = "Expected Years Lived Post Election",
         y = "Probability") + 
    scale_x_continuous(labels = scales::number_format()) +
    scale_y_continuous(labels = scales::percent_format()) +
    theme_classic()
...
fit_gov_5 %>% 
  ...
  ggplot(aes(diff)) +
    ...
    labs(title = "Posterior Probability Distribution",
         ...
         x = "Average Additional Years Lived Post Election: WA - SD",
         y = "Probability") + 
    scale_x_continuous(labels = scales::number_format(accuracy = 1)) +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    theme_classic()
...
The average value of the difference in years-to-live is almost certainly positive, with the most likely value being around `r round(coef(fit_gov_5)["stateWashington"] - coef(fit_gov_5)["stateSouth Dakota"], 0)` years. Notice that there is essentially zero chance that, on average, South Dakota candidates live longer.
...
First, we create a tibble with the desired input for our model. In our case the tibble has a variable named `sex` which contains two observations both with the value "Male". There is a `state` variable with the desired states. There is an `election_age` variable set to 10 years.
...
In words, we would predict that the candidate from Washington would live longer than the South Dakota candidate. By how much? Well, that number is an unknown parameter. By looking at our posterior above, our best estimate is about 13 years. However, it is important to note that it is possible that the male from South Dakota could outlive the Washington male. In fact, there is a 1 in 5 chance that he does so.

*Note what is the same and what is different when we move from a question about averages to a question about individuals.* In both cases, the most likely value is about the same. That is, the average behavior is the same as our expected value for any given individual. But the uncertainty is much greater for an individual prediction. The chance of the true average for Washington candidates being less than that for South Dakota candidates is essentially zero. Yet, for any individual pair of candidates, it would not even be slightly surprising for the South Dakota candidate to outlive the Washington candidate. Individuals vary. Averages never tell the whole story.
...
Expected values vary much less than predictions. The above chart makes that easy to see. We can be almost certain that the true underlying average for the numbers of years that Washington candidates live longer than South Dakota candidates is positive. But, for any two individual candidates, there is a good chance that that the South Dakota candidate will live longer. We can not ignore $\epsilon$ when predicting the outcome for individuals. When estimating expected values or long-run averages, the  $\epsilon$'s cancel out. 
