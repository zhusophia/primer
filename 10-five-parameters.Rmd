...
<!-- DK: Need a non-parallel lines plot, like the parallel lines plot in chapter 9. MF: Done. -->

<!-- Too much hard coding of numbers. Need to replace with coef(whatever)[2] MF: This seemed to be done already. -->

<!-- Clean up all the plots, especially the legends. Huh? -->

<!-- fit_gov_* is not a good naming scheme for the models. Should be like chapter 9, where we have fit_1, fit_2. MF: DONE-->
...
<!-- DK: Add a red circle to the graphic to show how we figured that out? MF: Done. -->
...
Now take a look at the coefficient for $c\_election\_age_i$, $\beta_2$. The median of the posterior, `r round(coef(fit_3)["c_election_age"], 1)`, represents the slope of the model. When comparing two candates who differ by one year in `c_election_age`, we expect that they will differ by `r round(coef(fit_3)["c_election_age"], 1)` years in `lived_after`. It makes sense that this value is negative. The more years a candidate has lived, the fewer years the candidate has left to live. So, for every extra year a candidate is alive before an election, their lifespan after the election will be `r abs(round(coef(fit_3)["c_election_age"], 1))` years lower, on average.
...
The coefficient for $c\_election\_age_i$, $\beta_2$, is `r round(coef(fit_4)["c_election_age"], 1)`. What does this mean? It is the slope for females. So, when comparing two female candidates who differ by one year in age, we expect that the older candidate will live `r abs(round(coef(fit_4)["c_election_age"], 1))` years less. Now direct your attention below at the coefficient of `sexMale:c_election_age`, $\beta_3$, which is `r round(coef(fit_4)["sexMale:c_election_age"], 1)`. This is the value that must be added to the coefficient of $c\_election\_age_i$ (recall $\beta_2 + \beta_3$) in order to find the slope for males. When the two are added together, this value, or slope, is about `r round(coef(fit_4)["c_election_age"] + coef(fit_4)["sexMale:c_election_age"], 1)`. When comparing two male candidates who differ in age by one year, we expect the older candidate to live about `r abs(round(coef(fit_4)["c_election_age"] + coef(fit_4)["sexMale:c_election_age"], 1))` years less. 
...
ggplot(ch10, aes(x = c_election_age, y = lived_after, color = sex)) +
  ...
  labs(title = "Interaction Model",
       ...
       x = "Years Lived (+ or - Average Age) at Time of Election", 
