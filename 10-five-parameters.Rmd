...
The **primer.data** package includes the  `governors` data set which features the demographic information about the candidates for governor in the United States. @barfort2020 gathered this data concluded that winning a gubernatorial election increases a candidate's lifespan.
...
Why do we use $y_i$ in this formula instead of $lived\_after_i$? Convention. The dependent variable is most regressions is referred to with the variable $y$, even though the outcome variable is rarely actually called $y$. However, it would not be usual to describe this model as:
