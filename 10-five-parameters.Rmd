...
<!-- DK: Need a non-parallel lines plot, like the parallel lines plot in chapter 9. -->

<!-- Too much hard coding of numbers. Need to replace with coef(whatever)[2] -->

<!-- Clean up all the plots, especially the legends. -->

<!-- Is fit_gov_* a good naming scheme for the models? -->



<!-- Get rid of duplicate names? Add more discussion below? Perhaps use as a good example of how the estimated posterior is not the true posterior because of the assumptions we are making about the structure of the error term. -->
...
*How long will two political candidates --- one male and one female, both 10 years older than the average candidate --- live after the election? How different will their lifespans be? Does winning the election affect their longevity?*

Note how far we have come in the *Primer*. These are sophisticated questions, involving issues of both prediction and causation. Yet, if we follow the Cardinal Virtues, we can provide sophisticated answers.
...
The **primer.data** package includes the  `governors` data set which features the demographic information about the candidates for governor in the United States. @barfort2020 gathered this data and concluded that winning a gubernatorial election increases a candidate's lifespan.
...
Consider someone who is about 40 years old on Election Day. We have a score or more data points for candidates around that age. Two died soon after the election. Some of them lived for 50 or more years after the election. *Variation fills the world.* But the fitted line tells us that, on average, we would expect a candidate that age to live about 37 years after the election. 
...
Looking at our results, you can see that our intercept value is around `r round(coef(fit_gov_3)["(Intercept)"], 0)`. The average female candidate, who had been alive the average number of years of all candidates, would live another `r round(coef(fit_gov_3)["(Intercept)"], 0)` years or so after the election. 
...
Now take a look at the coefficient for $c\_election\_age_i$, $\beta_2$. The median of the posterior, `r round(coef(fit_gov_3)["c_election_age"], 1)`, represents the slope of the model. When comparing two candates who differ by one year in `c_election_age`, we expect that they will differ by `r round(coef(fit_gov_3)["c_election_age"], 1)` years in `lived_after`. It makes sense that this value is negative. The more years a candidate has lived, the fewer years the candidate has left to live. So, for every extra year a candidate is alive before an election, their lifespan after the election will be `r abs(round(coef(fit_gov_3)["c_election_age"], 1))` years lower, on average.
...
* Our outcome variable is still $lived\_after_i$. We want to know how many years a candidate will live after an election. Our explanatory variables as the same as before. $male_i$ is zero for male candidates and one for female candidates. $c\_election\_age_i$ the number of years a candidate has lived before the election, relative to the average value for all candidates. In this model, we have a third predictor variable: the interaction between $male_i$ and $c\_election\_age_i$. 
...
$$ lived\_after_i =  \beta_0 + \beta_1 male_i + \beta_2 c\_election\_age_i + \\ \beta_3 male_i *  c\_election\_age_i + \epsilon_i$$
In both cases, $\beta_0$ has the same meaning. It is the average number of years lived after the election for women, who on the day of election, have been alive the average number of years of all candidates. Plug in 0 for $male_i$ and 0 for $c\_election\_age_i$ and both equations simplify to:
