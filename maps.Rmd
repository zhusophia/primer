...
There are two main sources we will be getting data from: the **American Community Survey (ACS)** and the **Decennial Census**. Both sources can be seen at [data.census.gov](https://data.census.gov).
...
**The first way:** look at the data.census.gov website. In addition to *Survey/Program*, you will also see the bolded term *Tables*. It will tell you the name of the variable. However, the variable names are a little tricky. For example, the variable name for population below is "B01003". **Make sure that the ID is a 6 digit string. If it isn't that is a Table ID and does not work.**
...
**The second way**: look at the census API documentation for information about the variables. By visiting [the API documentation](https://api.census.gov/data/2010/dec/sf1/variables.html), you can see the variables that the Census officially uses.
