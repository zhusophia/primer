...
First, God knows the ideal Preceptor Table, and so can calculate each individual's treatment effect, and any summary measure like the average. MF: Done 

Second, we have DGM Posterior, which is the posterior you would calculate if you had perfect knowledge of the data generating mechanism, meaning you have the correct model structure and exact beta values. With that, you could not be certain about any individual's causal effect, because of the Fundamental Problem of Causal Inference. But, you can still calculate posterior's for unknown numbers, and those posteriors would be perfect. MF: Done

Third, we have Your Posterior. It is not perfect! Be humble. Use with discretion. Don't be surprised when results are outside your 95% confidence intervals. In your mind, expand the confidence intervals. But this model is still better than hunches. MF: DONE.->
...
There's also the fact that even the ideal Preceptor Table is subject to errors. For instance, by the time we actually *do* the simple math to calculate the exact treatment effect of our reminders, lots of people in our table would have died --- yet they are included! Lots of people also would have crossed over into the voting age --- yet they are excluded. 
...
There exist three primary levels of knowledge possible knowledge in our scenario: God's Posterior (the ideal Preceptor Table), the DGM Posterior, and Our Posterior. 

#### God's Posterior

God's Posterior is the posterior if we had all the knowledge we need to *directly* answer our question. In short, God's posterior is built with the ideal Preceptor Table. God, equipped with the ideal Preceptor Table, can calculate each individual's treatment effect, and any summary measure we might be interested in, like the average treatment effect. 

This posterior represents the highest level of knowledge one can have --- with it, our questions merely require math. No estimation. 

#### DGM Posterior

The DGM Posterior is the posterior we would calculate if we had perfect knowledge of the data generating mechanism, meaning you have the correct model structure and exact beta values. 

With that, you could not be certain about any individual's causal effect, because of the Fundamental Problem of Causal Inference. As a brief reminder: we can never measure any one person's causal effect because we are unable to see a person's resulting behavior under treatment *and* control; we only have data on one of the two conditions. 
Regardless, we can still calculate posterior's for unknown numbers, and those posteriors would be perfect. If we went to our boss with our estimates from this posterior, we would expect our 95% confidence interval to be perfect. We would not expect for the resulting treatment effects to be far from our estimates at all.

#### Our posterior

Unfortunately, our posterior possesses even less certainty! In the real world, we *don't* have perfect knowledge of the DGM, the model structure, or the exact beta values. What does this mean? 
