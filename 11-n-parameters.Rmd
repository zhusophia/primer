...
First, God knows the ideal Preceptor Table, and so can calculate each individual's treatment effect, and any summary measure like the average. 

Second, we have DGM Posterior, which is the posterior you would calculate if you had perfect knowledge of the data generating mechanism, meaning you have the correct model structure and exact beta values. With that, you could not be certain about any individual's causal effect, because of the Fundamental Problem of Causal Inference. But, you can still calculate posterior's for unknown numbers, and those posteriors would be perfect. 

Third, we have Your Posterior. It is not perfect! Be humble. Use with discretion. Don't be surprised when results are outside your 95% confidence intervals. In your mind, expand the confidence intervals. But this model is still better than hunches. ->
...
As we have discussed before, the population *is not* the set of people, or voters, for which we have data. This is the dataset. Nor is it the set of voters about whol we would like to have data. Those are the rows in the ideal Preceptor Table. The population is the larger --- potentially much larger --- set of individuals which include both the data we have and the data we want. Generally, the population will be much larger than either the data we have and the data we want.  
...
Often in Wisdom, we consider the failings of our data and press on. However, this is a good time to consider what it really means to accept that our data is relevant to our question. With that in mind, let's break down our real, current question: 
