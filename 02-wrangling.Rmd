...
<!-- Explain where the csv file is located. Maybe use a different one? -->

<!-- Explain error = TRUE trick. -->
...
The `read_csv()` function turns flat files into data frames. More specifically, it turns files whose values are separated by commas (hence CSV). Let’s import a Comma Separated Values file that exists on the internet. The csv file `dem_score.csv` contains ratings of the level of democracy in different countries spanning 1952 to 1992. Let’s use the `read_csv()` function from the **readr** package to read it off the web, import it into R, and save it in a tibble called `dem_score`. 

```{r}
dem_score <- read_csv(file = "https://raw.githubusercontent.com/PPBDS/primer/master/02-wrangling/data/dem_score.csv")
```
