...
When giving variable names to our tibbles, there are some conditions. If the names of our variables are, or begin with, a number like `54` or include spaces like `my var`, you must include backticks around each name:
...
What is a distribution? Think of the distribution of a variable as an urn from which we can pull out, at random, values for that variable. Drawing a thousand or so values from that urn, and then looking at a histogram or density plot, can show where the values are centered and how the values vary. Because we are sloppy, we will use the word distribution to refer to both the (imaginary!) urn from which we are drawing values and to the list of values we have drawn. The vector of 115 ages in the `trains` data is a distribution.
...
Note that we are more likely to using the median and the mad to summarize a distribution. In this case, they are very similar to the mean and standard deviation.
...
<!-- Must add rowwise() and c_across(), given their use in chapter 7. -->
...
When importing datasets that have not already been extensively wrangled, there may be some observations in the tibble that are blank. These are called missing values, and they are often marked as `NA`.  
