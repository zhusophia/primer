...
If you have not taken Gov 50/1005, Read the Shopping Week section of The Primer and make sure your Git/Github is working. Read and watch the videos from [*Getting Used to R, RStudio, and R Markdown*](https://ismayc.github.io/rbasics-book/) by Chester Ismay and Patrick C. Kennedy. Also, check out [RStudio Essentials Videos](https://resources.rstudio.com/wistia-rstudio-essentials-2). Most relevant for us are ["Writing code in RStudio"](https://resources.rstudio.com/wistia-rstudio-essentials-2/rstudioessentialsprogrammingpart1-2), ["Projects in RStudio"](https://resources.rstudio.com/wistia-rstudio-essentials-2/rstudioessentialsmanagingpart1-2) and ["Github and RStudio"](https://resources.rstudio.com/wistia-rstudio-essentials-2/rstudioessentialsmanagingpart2-2).
...
Make sure the branch name is sensible. Again, this is a command that you only need to issue once, at least for our current workflow. (In more professional settings, you will often work on several different branches at once.) You should always be "on" this branch, never on "master". You can check this in the upper right corner of the git panel on R Studio.
...
Again, these commands bundle up a bunch of git commands (which you could do "by hand") into one handy step. This command does everything needed to create a "pull request" --- a request from you to me that I accept the changes you are proposing into the repo at PPBDS --- and then opens up the webpage to show you. BUT YOU ARE NOT DONE! You must PRESS the green button, sometimes twice. Until then, the PR has not actually been created. `pr_push()` just does everything before that. The "pr" in `pr_push()` stands for pull request.
...
1) After running `pr_pull_upstream()`, *you will often see a bunch of files in your Git tab in the top right corner of Rstudio* marked with an M (for Modified), including files which you know you did not edit. These are the files that have been updated on the "truth" --- on PPBDS --- since your last `pr_pull_upstream()`. Since you pulled them directly from the my repo, your forked repo sees all the changes other people have made and thinks that YOU MADE THEM. This is easily fixed, however — just commit all the changes to your forked repo. (Strangely, this seems to not always happen. If you don't see this effect, don't worry.)
...
4) `pr_push()` can be tricky. First, note that, if Preceptor has not accepted a (p)ull (r)equest which you have submitted, then your PR is still open. You can see it here: https://github.com/PPBDS/primer/pulls. In fact, you can see all the closed/completed pull requests as well. If, while one PR is still open, you submit another `pr_push()`, then this will just be added to your current PR. And that is OK! We don't need it to be separate. 
