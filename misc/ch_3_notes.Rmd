...
The problem with this version is that we want `prediction_game()` to *return* a message about the winner. Right now, it returns nothing. It just prints the winner. Let's change that, and also allow for guesses to be passed in as an argument.
...
```{r}
replicate(5, prediction_game(guesses = c(500, 600)))
```
...
```{r}
tibble(ID = 1:3) %>% 
  mutate(result = map_chr(ID, ~ prediction_game(guesses = c(500, 600))))
```
...
```{r}
tibble(ID = 1:3) %>% 
  mutate(samp = map(ID, ~ grab_a_sample(kenya, rv13, 25))) %>% 
