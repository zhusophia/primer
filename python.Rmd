...
```{r,include=FALSE}
# <!Atilla> Below notes are for Preceptor to successfully reticulate. 
#Working with reticulate is complex. for reticulate library to work properly on a machine, python should be in the same directory as Primer.
# After trying the above solution, there is no problem, you can write python codes without an error. If you get an error message saying you to install a miniconda, however, I recommend creating a directory and installing miniconda under that directory. Moreover, you will have to use "use_condaenv("path_of_conda_environment") to activate miniconda. 
library(reticulate)
use_condaenv("C:/MINICO~1/envs/r-reticulate")
```
