...
Additionally, representativeness can also consider whether or not the sample is biased or not. Let's look at it from the lenses of two types of missing data mechanisms: The assignment and sampling mechanism. The sampling mechanism will be talked about in this section, whereas the assignment mechanism will be discussed in the next section. 
...
A fourth assumption we use when working with causal models --- but not with predictive models --- is **"unconfoundedness."** Whether a unit received treatment or control should be independent of all covariates and all potential outcomes. Such a condition is most easily satisfied via random assignment to treatment or control.  

The idea of unconfoundedness can be seen through ‘The Perfect Doctor Experiment.’ Imagine we have this godly doctor who knows how any patient will respond to a certain drug. Using this information, they always assign each patient the best drug to cure their disease. Today, the doctor has just received a new drug ($Y_t(u)$) which should lower someone's blood pressure, and she is responsible for determining whether the drug actually works using an experiment. The information that the Perfect Doctor knows is displayed below. 
...
Let’s say the doctor was bribed to get results that showed that this new drug sucks, so they makes the following treatment assignments. 
...
The Perfect Doctor ends up producing skewed data, as they filters out all the bad reactions to the drug. Notice how the average causal effect (the difference between means) is significantly distorted. In the biased table, it shows that the control drug produces a better response, but when we look at the table with all the information, it shows that the new drug does better. We can also see how it impacts an individual unit, such as Yao, where The Doctor hides the negative effects of him taking the drug by assigning Yao be a part of the control group. 
...
The only way we actually care about is randomization, where you randomly assign control and treatment groups. Flip a coin, draw sticks, whatever. As long as you use randomization as your assignment mechanism, you’re good. There is the possibility that you can’t use pure randomization due to ethical or practical reasons, so we are forced to use non-random assignment mechanisms. This does result in skewed results, but it is encouraged that you use as much randomization as possible. 

<!-- DK: Delete this next section. But mention these topics in the above two sections, including the general idea of "Missing data mechanisms". -->

## Missing data mechanisms

*This section is a draft.*

If we have a Preceptor Table with no missing values, we have it easy. We just calculate the answer. Sadly, Preceptor Tables are (almost) always missing values, they have "missing data." The process by which some data is missing is known as the "missing data mechanism." The are two main missing data mechanisms which are of interest: the *assignment mechanism* and the *sampling mechanism*. The assignment mechanism is the process by which some units receive (or are "assigned") the treatment and some units receive the control. The sampling mechanism is the process by which some units appear in our data and some do not. 

<!-- DK: This section sucks! Delete it all!  -->
