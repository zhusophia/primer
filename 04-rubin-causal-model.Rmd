...
RCM: Overview --- matches the primer

gt --- Introduction to making tables with gt. This might be more like 45 minutes. Covers all the tricks that we use in the chapter, and some more stuff. Does not have anything to do with the chapter directly, so it might be assigned independently of it.

RCM: Postcards --- uses the old problem set with shaming data. Largely a review of everything in the chapter, but does not follow the chapter step-by-step like the Overview does.

RCD: Permutation Test --- explains the entire concept of permutation tests from start to finish, including the code to run one. Uses a set up (maybe the exact same set up, or one with a few extra rows) similar to the chapter, but there is no full discussion of permutation tests in the chapter. Key point is understand that Yao's value under control is something that we could have, in theory, observed but which, in fact, we did not.
...
A **Preceptor Table** is a table with rows and columns such that, if none of the data is missing, the thing we want to know is trivial to calculate. Preceptor Tables vary in the number of their rows and columns. We use question marks to indicate missing data in a Preceptor Table.
