...
Let's use the `nes` data from the **primer.tutorials** packages. (Explore the data on your own using `view()` and `glimpse()`. Read the help page by typing `?nes`.) The `nes` is data from [American National Election Studies](https://electionstudies.org/).
...
<!-- DK: Do we really need this? I don't think so! -->
...
```{r}
tibble(region = c("Midwest", "Northeast", "South", "West"),
       number = c(10072, 7378, 13680, 7428)) %>% 
  ggplot(aes(x = region, y = number)) +
    geom_col()
```

You will learn about the `tibble()` function and the `%>%` (pipe) operator in the next chapter.

Note how we must supply both an `x` and `y` aesthetic for `geom_col()`. We need a variable for the category, as with `geom_bar()`, and for the count of entries for each value, which `geom_bar()` calculates for us.
