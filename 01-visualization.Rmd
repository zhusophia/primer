...
<!-- DK: Would be nice trains has a variable with 3 or 4 levels. -->
...
Men are over-reprsented in the Control group even though the `treatment` was assigned at random.
...
Notice the gray section surrounding both sides of both of the lines we plotted. This area is called the confidence interval, which is set to a 95% confidence interval by default. We will learn about confidence intervals in Chapter \@ref(probability).
...
However, say you only need 2 of these variables, say `gender` and `treatment`. You can `select()`  these two variables:
