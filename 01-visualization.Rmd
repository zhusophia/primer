...
There are two distinct concepts: a distribution and a set values drawn from that distribution.  But, in everyday use, we apply "distribution" to both.

We use `summarize()` to calculate statistics for a variable, a column, a vector of values or a distribution. Note the language sloppiness. For the purposes of this book, "variable," "column," "vector," and "distribution" all mean the same thing. Popular statistical functions include: `mean()`, `median()`, `min()`, `max()`, `n()` and `sum()`. Functions which may be new to you include three measures of the "spread" of a distribution: `sd()` (the standard deviation), `mad()` (the scaled median absolute deviation) and `quantile()`. 
...
The scaled median absolute deviation, *mad*, is another measure of variation, although nowhere near as popular as the standard deviation. The formula for calculating *mad* is a bit mysterious.
...
<!-- DK: Finish with example usage, including na.rm, maybe? Move some of that discussion from chapter 2. -->

**`quantile()`**

The *quantile* of a distribution is the value of that distribution which occupies a specific percentile location in the sorted list of values. For example, the median occupies the 50th percentile. The minimum is at the 0th percentile. The maximum is at the 100th percentile.

```{r}
set.seed(9)
z <- rnorm(10000)
c(min(z), median(z), max(z))
quantile(z, probs = c(0, 0.5, 1))
```

The `probs` argument allows us to specify the percentile(s) we want. Two of the most important percentiles are the 2.5th and 97.5th.

```{r}
quantile(z, probs = c(0.025, 0.975))
```

The interval between these two percentiles includes 95% of all the values in the distribution. Depending on the context, this interval is sometimes called a "confidence interval." Different percentile ranges create confidence intervals of different widths. 

An interesting fact is that, most of the time, the 95% confidence interval is, roughly. the same as the center of the distribtion +/- two times the standard deviation.

```{r}
c(mean(z) - 2 * sd(z), mean(z) + 2 * sd(z))
```

Again, the match is only roughly true. But it is good enough for most applied work.


<!-- 
DK: Show how two standard deviations gets you out to the confidence interval. Make nice plot.
