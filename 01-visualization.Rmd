...
`mean()` and `sd()` are just two of the functions we can use within `summarize()`. Others include:

* mean(): the average  
* median(): the the average  
...
The median is another commonly reported measure of the center of vector of values, calculated by first sorting the vector from smallest to largest. The middle element in the sorted list is the *median*.  If the middle falls between two values, then the median is the mean of those two middle values. The median and the mean are the two most common measures of the center of a collection of numbers. The median is more stable, less affected by outliers. There is no widely accepted symbol for the median, although $\tilde{x}$ is not uncommon.

**`sd()`**

The *standard deviation* (*sd*) of a distribution is a measure of variation around the mean.  

$$\text{sd} = \sqrt{\frac{(x_1 - \bar{x})^2 + (x_2 - \bar{x})^2 + \cdots + (x_n - \bar{x})^2}{n - 1}}$$
<!-- DK: More detail. How much does it cover? Plot a weirdish distribution and show two sd's on it. -->
