...
<!-- Consider plot the rayshaders live. Key command may be  rgl::rglwidget(). -->
...
We might consider labeling the y-axis in plots of empirical distributions as "Proportion" rather than "Probability" since it is an actual proportion, calculated from real (or simulated) data. We will keep it as "Probability" since we want to emphasize the parallels between mathematical, empirical and posterior distributions.
...
The *posterior distribution* of electoral votes is popular topic, and an area of straong disagreement, among data scientists. Consider this posterior from *FiveThirtyEight*.
...
These elements of these vectors are all "draws" from the specified probability distributions. In most applied situations, our tools will produce draws rather than summary objects. Fortunately, a vector of draws is very easy to work with. Start with summary statistics:
...
Calculate a 95% confidence interval directly:

```{r}
quantile(draws$electoral_votes, probs = c(0.025, 0.975))
```

Approximate the 95% confidence interval in two ways:
...
<!-- DK: Worth rewriting? Yes! Tie it better to above and below. -->

### Independence

So far, you have learned how to explore $\text{Prob}(A)$, which is the fancy, statistical way of saying the probability distribution for event A. Keep in mind the distinction between an individual outcome from the set $A$, like the probability of heads (which is $\rho_h$) and the entire distribution $\text{Prob}(coin)$, which includes the chances of all possible outcomes.  
...
The probability of a random person having the disease is 0.6. Since they either have the disease or don't (those are the only two possibilities), the probability that they don't have the disease is 1 - 0.4.
...
How would you express this in statistical notation? $\text{Prob}(A|B)$ is the same thing as the probability of A *given* B. $\text{Prob}(A|B)$ essentially means the probability of A *if* we know for sure the value of B. You should also remember that $\text{Prob}(A|B)$ is not the same thing as $\text{Prob}(B|A)$.
...
```{r, echo = FALSE}
# Could create prevalence and accuracy variables as well. But not today!
...
# Is there an easier way of keep track of all these numbers? I build this object
# by first determining all the people that have the disease and those that
# don't. Then, within each category, determine who tests postive and who tests
# negative. Fortunately, I have figured out all those totals already. And I do
# an error check.

# DK: Why does a black dot appear in the middle of the sqaures?

# stopifnot(test_positive_true + 
#             test_negative_false == has_disease)
# stopifnot(test_negative_true + 
#             test_positive_false == city_pop - has_disease)
...
You are in a city of `r scales::comma(city_pop)` people. It is true that exactly 1% of the population has a disease, but we don't know which people have it and which do not. In terms of a count, that means 
`r scales::comma(has_disease)` people have this disease of the 
`r scales::comma(city_pop)` in the population.  We have a test which is 95% accurate. That is, if you have the disease there is a 95% chance that the test reports that you do, and the same if you don't have the disease.

So, of the `r scales::comma(has_disease)` people who truly do have the disease --- where `r scales::comma(has_disease)` is 1% of the 
`r scales::comma(city_pop)` people in the city ---  
`r scales::comma(test_positive_true)` would correctly test positive and `r scales::comma(test_negative_false)` would incorrectly test negative. The city also has `r scales::comma(city_pop - has_disease)` people who do not have the disease,  
`r scales::comma(test_positive_false)` of which incorrectly test positive and `r scales::comma(test_negative_true)` of which correctly test negative. That means that the number of people who would test positive for the disease is `r scales::comma(test_positive_true)` plus  `r scales::comma(test_positive_false)`, which equals 
`r scales::comma(test_positive_true + test_positive_false)` but 
*only `r scales::comma(test_positive_true)` of them have the disease*. In simple terms, the probability of you having the disease *given a positive test result* is much lower than the stated 95% accuracy of the test.
...
ggplot(pop, aes(x = as.factor(positive_test), 
...
Here is a joint distribution displayed in 3D. Instead of using the "jitter" feature in R to unstack the dots, we are using a 3D plot to visualize the number of dots in each box. The number of people who correctly test negative in this city is `r scales::comma(test_negative_true)`, far outnumbering any of the other categories. There are `r scales::comma(test_positive_false)` false positives, `r scales::comma(test_positive_true)` true positives, and only `r scales::comma(test_negative_false)` false negatives. This is why we can barely see the 3D bar coming from those combinations.
