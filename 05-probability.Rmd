...
odds_3rd <- tibble(p = sample(p_posterior$p, 
  ...
  mutate(heads = map_int(p, ~ rbinom(n = 1, 
                                     size = 20, 
                                     p = .))) %>% 
  mutate(above_ten = if_else(heads >= 10, 
                         TRUE, 
                         FALSE)) 
