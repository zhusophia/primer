...
Let's discuss the parallel structure of the 2-model, 3-model, and n-model case. First, we should always begin by discussing how to create the joint distribution of possible models and possible experimental results. This is done by, first, assuming that one of the models is true and then, second, show the variation in the world which results. That is, even if we know for a fact that you do not have the disease, there is still a chance that you test will come back positive. So, we should that for one model. Then, we creaet the joint distribution by doing that same thing sof all models. (We show all this code.) The joint distribution is a tibble which we are happy to look at. It always has two columns: the first is the possible models and the second is one possible result (which will vary) if that model is true. So, each possible model will have 1,000 (or at least 100) rows in the joint distribution.

Having create the joint distribution, we graph it in two dimensions. (We show the code for that.) Then, we graph it in three dimensions with rayshader. (Maybe there is a movie. Maybe we swoop in and fly around it.) Then, we take a "slice" of that joint distribution via rayshader. Move that slice around. Et cetera. Not sure how to do all that . . .

After rayshader, we then use the joint distribution to make two graphics, showing the posterior distribution for the model
...
# don't. Then, within each category, determine who tests positive and who tests
# negative. Fortunately, I have figured out all those totals already. And I do
# an error check.

# DK: Why does a black dot appear in the middle of the squares?
