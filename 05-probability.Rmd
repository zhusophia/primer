...
<!-- Code blocks need comments. Teach students something! Approximately as many lines of comments as lines of code, or at least 50% as many.  Done-->

<!-- Add ID variable to all data creates for Two Models, Three Models and so on. Done.-->

<!-- 3d plot for marble example is wrong. There is a 1/8 chance of getting 0 or 3 whites. There is a 3/8 chance of getting either 1 or 2. So, the 0 and 3 towers should be 1/3 the height of the 1 and 2 towers. -- Done.>

<!-- Did we do the N models case correctly, given the errors above? I am not sure. But we ought to make these cases as similar as possible, especially in terms of the code we show students.  Yes now. Done-->

<!-- It would be nice if the 3d graphics did not "pop up" when knitting the chapter.  Create a reproducible example, with the reprex package (or with a new repo with a simple Rmd), to show the problem. Then ask on RStudio. Done. Huge thanks to Ryan-->

<!-- Should discuss the concept of "population" in each example of probability distributions. The population is the imaginary urn from which our data has been, or will be, drawn. Check out Section 5.1. In fact, re-read the whole Cardinal Virtues section.  Done. -->

<!-- Explanation: There is a direct connection between the concept of the population and the concept of a probability distribution. But they are not the same thing! The connection is that the probability distribution --- whether it is empirical, mathematical, or posterior --- is a *description* of the contents of the urn, which is the population. Done. -->

<!-- How do we create a probability distribution for a given urn? Two approaches. First, assume that you know the data generating mechanism (DGM) --- the mathematical formula which describes the contents of the urn. Then just plot the formula. Second, if you don't know the DGM, you can just sample (a lot, you hope, and at random, you hope) from the urn. Plot what you get. And that is an estimate of the PDF. Done. -->
...
<!-- When discussing a parameter $p$ and a probability distribution p() in the absence of a specific context, then those simple terms are OK. But, whenever you are talking about specific parameters or specific probability distributions, you should have either subscripts or items in the parentheses, respectively.  Make this clear somewhere? -->
