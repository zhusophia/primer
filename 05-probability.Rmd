...
We use the symbol $\text{Prob()}$ to refer to the probability of a specific outcome. For example:

$$
\text{Prob(rolled die equal 1 or 2 or 3)} = 0.5
$$

We will rarely write out the full definition of the event with the $\text{Prob()}$ symbol.  Instead, we will define an event A as when a rolled die equals 1, 2 or 3. Then, we write 
...
As explained in Section \@ref(random-variables), R provides many functions for *simulating* random variables. We general refer to a (simulated) value of a random variable as a "draw." To create three "draws" from a random distribution, we use:
...
```{r}
ten_thousand_rolls <- sample(1:6, size = 10000, replace = TRUE)
table(ten_thousand_rolls)
```

Instead of keeping around a vector of length 10,000, we can just keep 10 values, without losing any information.

Two distributions can be identical even if they are of very different lengths. Let's compare our original distribution of 10 rolls of the die with another distribution which just features 1,000 copies of those 10 rolls.

```{r}
more_rolls <- rep(ten_rolls, 1000)
```
...
The two vectors have the exact same shape because even though of different lengths, these values were drawn from the same distribution. *The total count for each value does not matter. What matters is the relative proportions.*

### Normalizing distributions

If two distributions have the same shape, then they only differ by the labels on the y-axis. There are various ways of "normalizing" distributions to make them all the same scale. The most common scale is one which the area under the distribution adds to 1, e.g.,  100%. For example, we can transform either of the rolls plots above to look like:
...
$$ P(Y = y) = \begin{cases} 1/2 &\text{for }x= \text{Heads}\\ 1/2 &\text{for }x= \text{Tails} \end{cases}$$
