...
* On the right-hand side we first have the part contained in the model, consisting of two similar terms. Each term consists of a parameter and a data point. The betas are our two parameters: $\beta_1$ is the average age of Republicans in the population and $\beta_2$ is the average age of Democrats in the population.  $republican_i$ and $democrat_i$ are our explanatory variables and take the values 1 or 0. If someone is a Republican we have $republican_i = 1$ and $democrat_i = 0$, if someone is a Democrat we have $republican_i = 0$ and $democrat_i = 1$. In other words, their values mutually exclusive -- if you are a Democrat, you cannot also be a Republican.
...
* The small $i$'s are an index to number observations. It is equivalent to the "ID" column in our Preceptor Table and simply states that the outcome for person $i$ is explained by the modeled and non-modeled factors for person $i$. 
