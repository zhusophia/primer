...
fitted <- tibble(age = fitted(fit_obj)) %>% 
  ggplot(aes(age)) +
    geom_bar() +
...
posterior_predict(fit_obj, newdata = newobs) %>%
    as_tibble() %>%
    mutate_all(as.numeric)
```
...
<!-- DK: Does this section cause this warning? -->

<!-- Warning messages: -->
<!-- 1: Removed 2 rows containing missing values (geom_bar).  -->
<!-- 2: Removed 2 rows containing missing values (geom_bar). -->

<!-- I think it does. Definitely comes from this chapter. How to fix it? -->
