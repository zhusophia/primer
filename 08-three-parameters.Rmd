...
<!-- Also add a logistic model of liberal explained by income. (Need to put income in increments of 100,000.) Just barely significant! (Is this a three parameter model? I don't see an error term.) -->

<!-- Need to add a plot of fitted values and raw data. This will look a bit weird since, for each model, there are only two fitted values. Best is a jittered scatter plot with two columns, and then the fitted values put in the middle in red. -->
...
The key concept is the idea of a "population." From which larger population is the data we have being (conceptually) drawn? If we were only interested in the age of indidividuals in our data set, we would have no need for inference. *We know everyone's ages already.* We only need tools like `stan_glm()` if we seek to understand individuals not in our data. Only if our data is "representative," at least to some extent, of that larger population can we use our data to answer our questions.
...
```{r}
fit_obj_1 <- stan_glm(age ~ party - 1, 
                    data = trains, 
                    seed = 17,
                    refresh = 0)
```
...
trains %>% 
  select(age, party) %>% 
  mutate(fitted = fitted(fit_obj_1)) %>% 
  mutate(residual = residuals(fit_obj_1)) %>% 
  slice(1:8) %>% 
  gt() %>%
...
fitted <- tibble(age = fitted(fit_obj_1)) %>% 
  ggplot(aes(age)) +
    geom_bar() +
...
res <- tibble(resids = residuals(fit_obj_1)) %>% 
  ggplot(aes(resids)) +
    geom_histogram(bins = 100) +
    labs(x = "Residuals",
         y = NULL) 
...
```{r}
pp <- posterior_predict(fit_obj_1, newdata = new_obs) %>%
    as_tibble() %>%
...
posterior_predict(fit_obj_1, newdata = newobs) %>%
    as_tibble() %>%
    mutate_all(as.numeric)
```
...
```{r}
pp <- posterior_predict(fit_obj_1, newdata = newobs) %>%
    as_tibble() %>%
    mutate_all(as.numeric) %>% 
...
$$ y_i = \beta_0  + \beta_1 x_{d,i} + \epsilon_i$$
...
* The small $i$'s are an index for the data set. It is equivalent to the "ID" column in our Preceptor Table and simply states that the outcome for person $i$ is explained by the modeled and non-modelled factors for person $i$. The corresponding $x$'s have a $t$ or $c$ subscript, for Treated or Control.
...
```{r}
fit_obj_2 <- stan_glm(att_end ~ treatment - 1, 
                      data = trains, 
                      seed = 45,
                      refresh = 0)
...
fitted <- tibble(height = fitted(fit_obj_2)) %>% 
  ggplot(aes(height)) +
    geom_bar(width = 0.2) +
    labs(x = "Fitted Values",
         y = NULL) +
...
res <- tibble(resids = residuals(fit_obj_2)) %>% 
  ggplot(aes(resids)) +
    geom_histogram(bins = 50) +
    labs(x = "Residuals",
         y = NULL) 
...
pe <- posterior_epred(fit_obj_2, newobs) %>% 
    as_tibble() %>% 
    mutate(ate = `1` - `2`)
...
```{r}
pp <- posterior_predict(fit_obj_2, 
                        newdata = newobs) %>%
    as_tibble() %>%
    mutate_all(as.numeric) %>% 
    mutate(te = `1` - `2`)
...
pp %>% 
  ggplot(aes(x = te)) +
    geom_histogram(aes(y = after_stat(count/sum(count))),
                   bins = 100)  +
    labs(title = "Posterior Probability Distribution for Treatment Eff  ct",
...
So far, we have only created models in which the predictor variable is discrete, with only two possible values. `party` is either "Democrat" or "Republican". `treatment` is either "Treated" or "Control". Often times, however, the predictor variable will be continuous. Fortunately, the exact same approach works in this case as well. 
...
The mathematics with continuous predictor is unchanges from the intercept-including example we discussed above.

$$ y_i = \beta_0  + \beta_1 age_i + \epsilon_i$$

When comparing two people (persons 1 and 2), one aged one year older than the other, $\beta_1$ the expected difference in their incomes. The algebra is simple:
...
The use of `stan_glm()` is the same as above.
...
`liberal`, however, only takes on two values: TRUE and FALSE. In order to model it, we must use the `binomial` family. 


But first, following wisdom, let's look at our data.
...
In this chapter, we explored relationships between different variables in the `trains` data set. We built a predictive model and a causal model.
