...
<!-- Explore causal effects of models following chapter 18/19 in ROAS. Regress ending att on starting attitude and treatment effect. Show parallel lines. That is four parameters! Can also do the same thing with income and treatment. That will be a much worse model. Save interaction for five parameter chapter. MF: Done.-->

<!-- When interpreting a causal coefficient, start with: "The causal effect of ..." For other coefficients, start with "When comparing . . ." MF: Done. -->

<!-- Transforming the outcome variable. Give some more thoughts or advice. -->

<!-- No hard-coding numbers! r scales::comma(round(coef(fit_1_c)["(Intercept)"], 0)) MF: Done.-->

<!-- In all the examples, provide some clear interpretation sentences. Set them off with skipped lines, and maybe in italics. -->

<!-- Expand rge "interpreting coefficients" section. Difference between across-unit and with-in unit. Causal versus predictive. Unless you have a set up with two or more potential outcomes, you can't do causal. You can't do intra-unit. 

Whenever the setting is predictive --- that is, whenever it is not causal --- you are always comparing two different units (or groups of units). You are never considering a single unit. 

Also, does not work in logistic. But divide by four rule. -->

<!-- Clean up General guidelines for selecting variables. Reasons 1 and 3 seem similar.  -->
...
<!-- DK: This is not great. Need an example. Does it really belong here. If not, then where? -->
...
How do we decide which variables are useful? First, let's look at our intercept. The 95% confidence interval for  `att_end` is equal to the coefficient-- `r scales::comma(round(coef(fit_1_model)["(Intercept)"], 1))`-- plus or minus two standard errors.
...
The use of `stan_glm` is the same as usual. Using `stan_glm`, we will create our model, `fitted_1`. 

```{r}
fitted_1 <- stan_glm(formula = att_end ~ liberal + att_start,
                  data = trains,
                  refresh = 0,
                  seed = 42)
...
<!-- MF: The method above was taken from this article on your page: https://www.davidkane.info/files/modern-dive-6.4. Should we show the geom_parallel_slopes method as well? (BELOW) --> 

We can also use a shortcut function `geom_parallel_slopes` from the `moderndive` package to accomplish a similar plot. This plot, however, is Frequentist, not Bayesian.

```{r}
trains %>% 
ggplot(aes(x = att_start, y = att_end, color = liberal)) +
  geom_point() +
