...
fit_obj %>% 
  ...
  ggplot(aes(x = mu)) +
    ...
    labs(title = "Posterior Probability Distribution",
         subtitle = "Average height among American adult men",
...
fit_obj %>% 
  ...
  ggplot(aes(x = sigma)) +
    geom_histogram(aes(y = after_stat(count/sum(count))), 
                   bins = 100) +
    labs(title = "Posterior Probability Distribution",
         subtitle = "Height standard deviation among American adult men",
...
cov_analysis <- tibble(ID = 1:n) %>% 
  ...
  # matter since we are not using its value in the function call to sample_n().
  ...
  # when given a fitted model and a paramter, returns that confidence interval
  ...
  # the lower and uppper bound into separate columns and then, after doing so,
...
pe %>% 
  ggplot(aes(x = `1`)) +
    geom_histogram(aes(y = after_stat(count/sum(count))),
                   bins = 100) +
    labs(title = "Posterior Probability Distribution",
         subtitle = "For the probability that a 180 cm tall person is female",
         x = "Probability",
         y = "Probability",
         caption = "Data source: NHANES") + 
