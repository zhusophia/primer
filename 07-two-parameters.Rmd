...
<!-- DK: Easier way to code this? More explanation of the coding tricks? -->
...
Variables with well-behaved, continuous ranges are the easiest to handle. We started with `height` because it was simple. Sadly, however, many variables are not like `height`. Consider `diabetes`, a variable in `nhanes` which takes on two possible values: 0 (if you do not have diabetes) and 1 (if you do). In the same way that we would like to construct a model which explains or predicts `height`, we would like to build a model which explains or predicts `diabetes`. 

<!-- DK: Discuss cardinal virtues? -->



Before we do that, however, let's revisit the urn example from Chapter \@ref(one-parameter). 
