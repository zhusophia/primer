...
<!-- The distribution of the hat things is, more or less, your posterior. MF: Done. -->

<!-- sqrt from hierarchy should be math symbol, not R function.  MF: Done.-->

<!-- Add this Q&A below the hierarchy. And clean it up! MF: Done.-->

<!-- "I thought that MADs were the same thing as standard deviations. Now you say they are the same things as standard errors. Which is it?" MF: Done. -->

<!-- MADs and standard deviations are, more or less, the same thing. They are both measures of the variability of a distribution. In most cases, they have very similar values. A *standard error* is also a standard devitaion. It is just the standard deviation of the distribution of the estimates, and that distribution of estimates is, more or less, your posterior. So, we can use MAD to describe that distribution. It just does not have a special name, like standard error.  MF: Done. -->

<!-- default comment formating is fine. Mostly fixed. MF: Done.-->

<!-- Show post_dist for n > 8 as whole dist with n > 8 colored red. Not >= 8. MF: Done.-->

<!-- format of Summary section. Use better math symbols. MF: Done. -->

<!-- Photo at the end of Evy or . . MF: Done. -->

<!-- Purpose of two versions of Joint Distribution graphic. Maybe at least change color of key changes in second one in order to highlight why we are seeing this twice? Certainly add $p$ to the y-axis label, if not also to the tile. MF: Done.-->

<!-- Hypothesis test section. More detail? More like Khan Academy? Reference Gelman? "Null hyppthesis test." MF: Done. -->

<!-- Outlier discussion in Temperance not quite right. Model uncertainty generally does not cause us to change our the center of the posterior. It causes us to expand the MAD. Donald Rumsfeld (and Donal Rubin) say: "Get MAD!" Or maybe not. MF: Done.-->

<!-- Asserting a posterior distribution is, conceptually, the same as opening a casino and allowing people to place wagers on outcomes, with the odds consistent with your posterior. If your posterior is wrong, they will take all your money. That would be bad! MF: Done.-->

<!-- Rule of thumb: The world in the future is always more variable than our models would suggest. The entire point of the virtue of Temperance to be aware of that fact and take account of it. Courage --- that bold bastard --- comes along and says, "Here is the model! Let's calculate the posterior and open the casino!" Temperance --- quite the old woman from the point of view of that idiot Courage --- says, "Hold on, stupid! Yes, I can calculate the posterior, but we need to be cautious." Temerance is concerned with the tails, not the center. Limit the bets on the first few nights. Make sure our posterior is OK. Temperance is about the sensible management of the casino.  MF: Done. -->
...
<!-- DK: Clean up this text. Give a better sense of the whole chapter. MF: Done.-->
...
The urn below has a certain number of red and a certain number of white beads all of equal size, mixed well together. What proportion of this urn's beads are red? That is, what is the value of $p$?
...
To begin this chapter, we will look at a real sampling activity: the urn. We will build important conceptual knowledge that will aid us as we move into more complex examples. Next, we will simulate the urn example using our virtual tools and knowledge. These tools will help us automate the long process of sampling, and better understand the way in which we make predictions. 

After this, we will attempt to understand *standard error* and the ways in which uncertainty factors into our predictions. In this section, we will also learn some important terminology related to sampling and data science. As we enter the second half of our chapter, we will answer the two questions above using our new understanding. 

Finally, we will have a discussion section to go over important topics such as case studies, our sampling mechanism, and the difference between precision and accuracy. We will end with a summary that includes the most important takeaways from each section of this chapter. Good luck!
...
# called within map(), we are performing sample_n() upon our urn once (ID = 1)
# and taking a sample of 50 beads.
...
tibble(ID = 1) %>% 
  ...
  # Using map_int, perform the sum where (within the urn -- denotoed by the
...
<!-- DK: Clean up this code. Discuss more? MF: Done.-->
...
Once again, the most frequently occurring proportions of red beads occur between 35% and 45%. Every now and then, we obtain proportions as low as between 20% and 25%, and others as high as between 55% and 60%. These are rare, however. The missing buckets at .2 and .65 occur due to the fact that, with our urn, extreme highs and lows are infrequent -- occasionally, they do not occur at all. Furthermore, observe that we now have a much more symmetric and smoother bell-shaped distribution. This distribution is, in fact, approximated well by a normal distribution. 

<!-- DK: Discuss why we have missing buckets. MF: Done. -->
...
joint_dist %>%
  ggplot(aes(y = p, x = red_in_paddle)) +
    ...
    labs(title = expression(paste(
      "Joint Distribution of Red Beads in Paddle and Proportion Red Beads in Urn", 
      hat(p))), 
         x = "Number of Red Beads in Paddle",
