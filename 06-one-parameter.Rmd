...
<!-- Switch the urn to 1,000 beads to make the math easier and the code quicker. Use 400 red beads. Do not use number of red beads as unknown parameter. MF: DONE. 

Use p throughout. Introduce $\hat{p}$ earlier and then use it consistently. That sets the stage for the standard error. MF: DONE. 
...
<!-- End of 6.1: If you can only keep around two numbers about that distribution, what would they be? You need a measure of the center, maybe mean or median. You need a measure of the spread, sd or mad. Summary statistics for a distribution, for when you can't keep the whole thing around, or show the pretty graph. Do those two numbers tell you everything about a distribution? No! But, if you only get to keep two numbers, those are the probably ones you want. MF: DONE BUT placed in 6.3 instead, since we hadn't covered summary statistics yet. I can modify and place at the beginning if you prefer it to come up earlier. -->

<!-- 6.3  must explain clearly the concept of standard error, p-values and confidence intervals. Need this rock-solid before going into MAD SD complexity in chapter 7. MF: Are we still incorporating p-values here? -->
...
<!-- **The standard error is, more or less, the standard deviation of the posterior probability distribution for p.** So, +/- two standard errors is about the same as a 95% confidence interval. MF: DONE-->
...
<!-- **The mean or median is a good estimate for the center of the posterior and the standard error or mad is a good estimate for the variability of the posterior, with +/- 2 standard errors covering 95% of the outcomes.** MF: DONE-->
...
Observe the urn in Figure 6.2. It has a certain number of red and a certain number of white beads all of equal size. It appears the urn has been mixed beforehand, as there does not seem to be any coherent pattern to the spatial distribution of the red and white beads.
...
*If we get 17 red beads in a random sample of size 50 taken from an urn with 2,400 (white or red) beads, how many red beads are in the urn?*

*What is the probability, using the same urn, that we will draw more than 8 red beads if we use a shovel of size 20?*

<!-- ADD A SECTION BRIEFING THE REST OF THE CHAPTER -->
...
<!-- DK: Clean up, make public and explain this code. MF: Done. -->

```{r, fig.cap = "Comparing the distributions of proportion red for different sample sizes."}
...
MF: I'm confused why we are using (2 * sqrt(.x)). If we are plotting for standard error, shouldn't we have the standard deviation (prop_red_sd) over sqrt of the sample size, x? -->
...
<!-- MF: I took from explanations online, but still feel SE isn't crystal clear. Need to revisit. -->
...
<!-- REWRITE THIS ENTIRE SECTION TO REFLECT FOCUS ON STANDARD ERROR. -->

<!-- What was the value of the population proportion $p$ of the $N$ = 1000 beads in the actual urn that were red? There were 400 red beads, for a proportion red of 400/1000 = 0.375 = 37.5%! How do we know this? Did the authors do an exhaustive count of all the beads? No! They were listed in the contents of the box that the urn came in! Hence we were able to make the contents of the virtual `urn` match the tactile urn:
...
The sampling activity involving the urn is merely an *idealized version* of how sampling is done in real life. 
...
<!-- INSERT RECAP FOR EACH SECTION WITH 2-3 IMPORTANT PIECES OF INFORMATION -->
