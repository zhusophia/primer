...
<!-- Change axis labels as you like. Make better use of pedagogical opportunities. MF: Done.-->

<!-- Add code comments (if you want) which explain some of your choices. Think of them like footnotes. MF: Done.-->

<!-- If you like, improve the quality of the graphics as you go along. MF: Done (sparingly).-->

<!-- THIS IS YOUR CHAPTER! Put multiple bad standard error discussions everywhere! MF: Done.-->

<!-- Delete all old code that you are not using. MF: Done.-->

<!-- Standard error is just a fancy term for your uncertainty about something you don't know. Standard error == my (uncertain) beliefs. Tell me your standard error and then I know your 95% confidence interval.-->

<!-- --------------------- -->

<!-- Do this like Maslow -->

<!-- Preceptor's Seven Level Hierarchy of Teaching Knowledge Good -->

<!-- Gif -->

<!-- Standard errors are just the way that old people talk about confidence intervals. -->

<!-- Standard error == my (uncertain) beliefs. -->

<!-- Standard error is just a fancy term for your uncertainty about something you don't know. -->

<!-- The standard error is the standard deviation of any collection of hat things. -->

<!-- p -> to $\hat{p}$ to the standard error of $\hat{p}$ to our posterior probability distribution for p. -->

<!-- Math (put in actual math formula) -->

<!-- --------------------- -->

<!-- Replace 6.1 (and other hard-coded numbers) with reference/label. MF: DONE. -->

<!-- Add longer figure captions, at your discretion. Student should be able to look at figure, read its caption, and understand the whole thing. MF: Done.-->

<!-- Fix 816 -->

<!-- Show code for final plots. Add comments to that code. MF: Done except for one overly complicated plot. -->

<!-- Explain stan_glm usage! -->

<!-- Find a gif for: "Standard errors are just the way that old people talk about confidence intervals." -->

<!-- 

"The standard error is the standard deviation of any collection of hat things." "... any collection of estimates." 

, which is 40%. There is only one p.

$\hat{p}$ is the estimated value of p which comes from taking a sample. There can be billions and billions of $\hat{p}$'s. 

*********
p -> to $\hat{p}$ to the standard error of $\hat{p}$ to our posterior probability distribution for p.
*********

They must understand what the standard error of $\hat{p}$ means. They do not need to understand why.

We do this to set the stage for continuous variables in chapter 7. Also, strictly speaking, the stan_glm() model assumes that p is continuous. -->


<!-- 6.2 Add a scatter plot of the results. -->

<!-- The plot the means -->

<!-- 6.2 should do a lot of samples, then calculate the mean of those samples and the standard deviation of those samples. (Explain why?) And then the cool graphics are the standard deviation decreasing and also mean estimates more stable. Maybe use 100 times instead of 1000. That will show mean converging as shovel size increases. Maybe make it a gganimation? -->

<!-- 6.3  must explain clearly the concept of standard error and confidence intervals. Need this rock-solid before going into MAD SD complexity in chapter 7. -->

<!-- 6.3 statistical definitions. Expand to include standard error and posterior probabilty distribution. (Rename as the posterior probablity distribution for p. Show them that posterior.) There is a truth! There is a true value for p which we do not know! We want to create a posterior probability distribution which summarizes our knowledge. (I am not sure if the words "sampling distribution" belong beyond a single sentence.) We care about the posterior probability distribution of p. We will make that, formally, in the next section. For now, here is a preview. The center of that distribution is around the mean or median of the proportion in your sample. (Which makes sense! Show how this line goes through your mean dots.) The sd (or mad) of that posterior is this formula! Give standard error formula. Note that this is the same thing as the standard deviation of the repeated samples! (Or at least say that, and then show how the red line goes through your dots.) -->


<!-- 6.3 finishes with this. -->

<!-- "Formula for the standard error. Not justified! How do we calculate a standard error if we only take one sample?!!! -->

<!-- Then provide a demonstration of this! (Next summer!) Simple simulation with urns of various sizes, percentage of red beads which varies, grab a shovel of different sizes, draw a sample, calculate a standard error, calculate a confidence interval, test whetherr ci includes the truth. 95% of the time it should. -->

<!-- 6.4 shows the right way -->
...
- All children ages 12+ are entered into the lottery. 
...
Observe the urn in Figure \@ref(fig:urn_pic_one). It has a certain number of red and a certain number of white beads all of equal size. It appears the urn has been mixed beforehand, as there does not seem to be any coherent pattern to the spatial distribution of the red and white beads.

Let's now ask ourselves, what proportion of this urn's beads are red? 

```{r urn_pic_one, echo = FALSE, fig.cap = "An urn with red and white beads."}
...
*If we get 17 red beads in a random sample of size 50 taken from an urn with 1000 (white or red) beads, how many red beads are in the urn?*
...
Instead of performing an exhaustive count, let's insert a shovel into the urn as seen in Figure \@ref(fig:urn_pic_two). Using the shovel, let's remove $5 \cdot 10 = 50$ beads, as seen in Figure \@ref(fig:urn_pic_3). Here, we are taking a *sample* of the total population of beads. 

```{r urn_pic_two, echo = FALSE, fig.cap = "Inserting a shovel into the urn."}
knitr::include_graphics("06-one-parameter/images/sampling_bowl_2.jpg")
```


```{r urn_pic_3, echo = FALSE, fig.cap = "Removing 50 beads from the urn."}
...
tactile_sample_urn %>%
  ...
  # Add scale_y_continuous with breaks by 
  # 1, as the default shows the y-axis from
  # 1 to 10 with breaks at .5. Breaks by 1 
  # is better for this plot, as all resulting 
  # values are integers. 
  
  scale_y_continuous(breaks = seq(from = 0, to = 10, by = 1)) +
  
  # The call expression() is used to insert a mathematical
  # expression, like p-hat. The paste after expression allows 
  # us to paste text prior to said expression. 
...
`r scales::comma(nrow(urn))` beads in our virtual urn. And that is OK! `ID` means the same concept in both cases. We will need to be careful, however, when working with more than one table which has an `ID` column. In that case, we will rename one of the columns in order to avoid conflicts.   
...
# Define ID as 1 to look at one example of drawing
# fifty beads. When ID is called within map(), we
# are performing sample_n() upon our urn once (ID
# = 1) and taking a sample of 50 beads.
...
Let's visualize this variation in a histogram in Figure \@ref(fig:histogram).

Note that we add 'binwidth = 0.05' and 'boundary = 0.4' arguments as well. Recall that setting 'boundary = 0.4' ensures a binning scheme with one of the bins' boundaries at 0.4. Since the 'binwidth = 0.05' is also set, this will create bins with boundaries at 0.30, 0.35, 0.45, 0.5, etc. as well. Recall that $\hat{p}$ is equal to the proportions red in our samples. 
...
ggplot(aes(x = prop_red)) +
  ...
  # Labels the axes in an attractive fashion. Without this code, 
  # the axes labels would include 2.5, which makes no sense because
...
```{r, fig.cap = "Comparing 33 virtual and 33 tactile proportions red. Note that, though the figures differ slightly, both are centered around .35 to .40. This shows that, in both sampling distributions, the most frequently occuring proportion red is between 35 and 40%."}

# bind_rows() takes our data from virtual_samples and 
# "binds" it with our data from tactile_sample_urn, resulting
# in one set of data for us to work with. Mutating to create
# the "type" column allows us to see which pieces of data come
# from virtual sampling and which come from tactical sampling.
...
facet_compare <- bind_rows(virtual_samples %>% 
  ...
  # Use mutate() to alter the column factor into a factor
  # column, rather than the default character column. Calling
  # levels() establishes our two "factors". This step is 
  # here to help R interpret this column for when we perform 
  # our facet_wrap(). 
  ...
  # facet_wrapping by type creates two different graphs
  # on our plane. The data is separated into two graphs 
  # based upon the column after the ~. 
  
    facet_wrap(~ type) +
  
  # Add an x axis with p-hat using expression() with paste()
  # for clarification on p-hat's meaning. Add an appropriate 
  # title and inspect our creation. 
...
Observe that we now have 1,000 replicates of `prop_red`, the proportion of 50 beads that are red. Using the same code as earlier, let's now visualize the distribution of these 1,000 replicates of `prop_red` in a histogram in Figure \@ref(fig:hist_distribution).
...
For easy comparison, we present the three resulting histograms in a single row with matching x and y axes in Figure \@ref(fig:multiple_hist).

```{r multiple_hist, fig.cap = "Comparing the distributions of proportion red for different sample sizes (25, 50, 100). The important takeaway is that our center becomes more concentrated as our sample size increases, indicating a smaller standard deviation between our guesses."}
...
```{r, fig.cap = "Comparing the number of slots in the shovel with the standard deviation of proportions red. Here, we see that SD decreases with higher sample sizes. This indicates that larger sample sizes yield more precise estimates."}
...
shovels <- tibble(shovel_size = c(25, 50, 100)) %>%
  ...
  # Use map_dbl() to create a column that draws the 
  # standard deviations of our prop_red values. We 
  # can use pull after our ~ to isolate the prop_red
  # values and use a pipe (as we normally do) here
  # to draw the sd() of this column. 
...
MF: I'm confused why we are using (2 * sqrt(.x)). If we are plotting for standard error, shouldn't we have the standard deviation (prop_red_sd) over sqrt of the sample size, x? DK: I don't know! Math is hard! -->
...
*SE refers to the standard deviation of a sample statistic (aka point estimate), such as the mean or median.* For example, the "standard error of the mean" refers to the standard deviation of the distribution of sample means taken from a population. 
...
If this sounds confusing, don't worry! It should. Before we can explain this in more depth, it is important to understand some terminology pertaining to standard error. 
...
Eighth, we say **biased sampling** occurs if certain individuals or observations in a population have a higher chance of being included in a sample than others. We say a sampling procedure is *unbiased* if every observation in a population had an equal chance of being sampled. Had the red beads been much smaller than the white beads, and therefore more prone to falling through the shovel, our sample would have been *biased*. In our sampling activities, since we mixed all $N = 1000$ beads prior to each group's sampling and since each of the equally sized beads had an equal chance of being sampled, our samples were *unbiased*.
...
To help reinforce these concepts, let's re-display Figure \@ref(fig:compare_p_hat) but using our new terminology, notation, and definitions relating to sampling in the figure below.

```{r compare_p_hat, echo = FALSE, fig.cap = "Three sampling distributions of the sample proportion $\\hat{p}$. Note the increased concentration on the bins around .4 as our sample size increases."}
...
<!-- MF: Add more SE discussion here. -->

Before we move on from our urn, do we know what the true value of the population proportion $p$ of the $N$ = 1000 beads in the urn was? Yes, actually. There were 400 red beads, for a proportion red of 400/1000 = .40 = 40%! How do we know this? Did the authors do an exhaustive count of all the beads? No! They were listed in the contents of the box that the urn came in! Hence we were able to make the contents of the virtual `urn` match the tactile urn. 
...
The sampling activity involving the urn is merely an *idealized version* of how sampling is done in real life. This is *not* how sampling is done in real life. In a real life scenario, we won't know what the true value of the population parameter is. Furthermore, we wouldn't be able to take 1,000 replicated samples. Rather, we take a single sample that's as large as we can afford. In the next section, we'll study a real life example of sampling: polls. 
...
Recall the question we asked at the beginning of our chapter: 
...
Recall Question #1 from the start of the Chapter: "If we get 17 red beads in a random sample of size 50, how many red beads are in the entire urn?" The answer is not a single number. Our posterior distribution is just that: a distribution. From our sample, we have many different results for the total number of red beads in the entire urn. Certain totals of red beads, like the extremes close to 0 or 1000, are essentially impossible due to our sample value being 32%. Some totals, like 400, occur far more frequently than other totals. 
...
<!-- MF: So, we just want to show stan_glm here, right? No explanations? -->

Instead of building the joint and conditional probability distributions "by hand" as above, we can just use `stan_glm()` from the **rstanarm* package. For now, we will just walk through the code for doing this very quickly. Chapter \@ref(two-parameters) will provide a step-by-step explanation.

Assume that, as before, a shovel of size 50 drawn from the urn has returned 17 red beads and 33 white. Create an object which contains this data.
...
```
 ...
 `shovel` is a tibble with two variables: `ID` and `red`, which takes the value 1 if that bead is red, and the value 0 if it is white. Create a model, using `stan_glm()`. 
...
This is, more or less, the same result we got in the earlier plot, but with much less work.

It is just as easy to estimate the posterior probability distribution of the number of red beads which will be pulled from the urn with a shovel of size 20, given the information we already know about the composition of the urn.
...
We can either eyeball this graphic or calculate the value from the `new_shovel` object directly:

```{r}
sum((new_shovel$total > 8)/ length(new_shovel$total))
```


<!-- DK: Clean up above. Match it more closely to by-hand version.  -->
...
To finish our chapter, we will summarize each section and emphasize the most important takeaways. 
...
- Sampling is preferable in the real-world because it is often impossible to sample "all the beads" (all the people) in a population. 
- With sampling, you see variations in your results. This is known as sampling variation and is expected, especially if you draw samples at random (unbiased).
...
- Standard error is just a fancy term for your uncertainty about something you don't know. Standard error == our (uncertain) beliefs. 
...
- We find that larger sample sizes = lower standard errors = more accurate and representative guesses. 
...
### Answering the questions
<!-- MF: ADD SUMMARY FOR ANSWERING THE QUESTIONS AFTER RE-READING AND EDITING. -->
