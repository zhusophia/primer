...
<!-- Need some discussion of the Cardinal Virtues, even just a brief reminder in the Discussion section. MF: Should this go through each virtue and apply it to the urn? Or should we only mention the virtues? Yes. In 6.4.4, about 4 paragraphs. MF: Done.-->

<!-- All references to estimating the number of red beads should be change to estimating p, the proportion of red beads.  Or, just make clear a couple of times that they are the same thing. And that we "pretend" that p is continuous even though, stritcly speaking it is not. MF: Done.-->

<!-- Fix up Maslow. MF: Done.-->

<!-- Put these two in the Summary: MF: Done.-->

<!-- Does this belong in the Summary? There is a truth! There is a true value for p which we do not know! We want to create a posterior probability distribution which summarizes our knowledge. We care about the posterior probability distribution of p.  The center of that distribution is around the mean or median of the proportion in your sample. The sd (or mad) of that posterior is this formula! Give standard error formula. Note that this is the same thing as the standard deviation of the repeated samples! MF: Done.-->
...
<!-- Put Hyppthesis section right before using the posterior. Give a clear two paragraph description of hyppthesis test. "Traditionally, most scientific papers are not so much interested in estimating p. They are interested in testing specific hypotheses." Explain it exactly the way that Khan explains it. MF: Done.-->

<!-- Cover most of the points here: https://ppbds.github.io/primer/five-parameters.html#testing. One hypothesis: There are equal number of red and white beads in the urn. Can we reject that hypothesis? Convention: if the 95% confidence interval excludes the null hypthesis, then we reject it. Otherwise, we don't reject it. (Not we accept it.) MF: Done. -->

<!-- Who cares! Rejecting or not rejecting hypotheses never helps us to answer real questions. In general, null hypthoses are never true. A big enough will always reject the null hypothesis. So, it is not that interesting.  MF: Done. -->

<!-- Also, no need for sample_n(10) when looking at post_dist, I think. So, just look at first ten. Then discuss the size of post_dist, which will be some random number like 2,567. MF: Done. -->

<!-- Add a second posterior plot with all the results with red > 8 colored red. *Each question requires looking at a new area under the curve.* When someones asks you a question, they are doing two things. First, they are providing instructions as to the posterior your should create. (Shovel with 20 spots.) Second, they are asking a question about the area under the curve in a specific region. MF: Done.-->
...
This scene is from  *The Hunger Games*, a dystopian novel in which children are selected via lottery to fight to the death. Primrose Everdeen is selected from the urn. Why does she have the misfortune of being selected? Or, as we will refer to it henceforth, sampled?
...
The urn has a certain number of red and a certain number of white beads all of equal size, mixed well together. What proportion of this urn's beads are red? That is, what is the value of $p$?
...
Note that we add `binwidth = 0.05` and `boundary = 0.4` arguments as well. Setting `boundary = 0.4` ensures a binning scheme with one of the bins' boundaries at 0.4. Since `binwidth = 0.05` is also set, this will create bins with boundaries at 0.30, 0.35, 0.45, and so on. Recall that $\hat{p}$ is equal to the proportions red in our samples. 
...
knitr::include_graphics("06-one-parameter/images/revised_preceptor.png")
```
...
Recall our view on hypothesis tests: **Amateurs test. Professionals summarize.** Traditionally, most scientific papers are not so much interested in estimating p. They are interested in testing specific hypotheses. But, how useful is that method?

Let's look at a possible hypothesis in our urn paradigm: there are equal number of red and white beads in the urn. Can we reject that hypothesis? Convention: if the 95% confidence interval excludes the null hypothesis, then we reject it. Otherwise, we don't reject it. However, this does not mean that we accept it. More to the point: rejecting or not rejecting hypotheses never helps us to answer real questions. A Yes/No question throws away too much information to (almost) ever be useful. There is no reason to test when you can summarize by providing the full posterior probability distribution, as we will perform below.
...
# Define a paddle size for our
# joint distributions

paddle_size <- 20

# To create our posterior distribution,
# filter for where the red in paddle 
# is equal to 17. This takes our slice.

post_dist <- joint_dist %>% 
  filter(red_in_paddle == 17) %>% 
  
  # Remove the columns of red_in_urn
  # and red_in_paddle so we are left 
  # with p.
  
  select(-red_in_urn, -red_in_paddle) %>% 
  
  # Create object new_reds which 
  # maps p using rbinom with size
  # defined as our previously created
  # paddle_size of 20. 
...
When we think of Temperance, we should think of uncertainty. Our models are never as accurate as we believe them to be. Would we really like to bet on our finding? That  `r round(100 *sum(post_dist$new_reds > 8) /  length(post_dist$new_reds))`% of the time, we will get more than 8 red beads from our sample of size 20? The truth is, outliers are *much more common* than we might think. Unknown unknowns, prediction uncertainty, the ever changing world -- all of these factors should be a warning to us: always expect the unexpected.
...
This posterior is similar to the one we created, much more laboriously, above. 
...
**p (the truth) -> to $\hat{p}$  (my estimate) -> to the standard error of $\hat{p}$ (black box of math mumbo jumbo and computer simulation magic) -> to our posterior probability distribution for p (our beliefs about the truth).**
...
As we continue our journey, recall the case of Primrose Everdeen and what she represents: no matter how realistic is our model is, our predictions are **never certain**.  "May the posterior be ever in your favor!"
