...
In Section \@ref(sampling-activity), we performed a *tactile* sampling activity. We used a physical urn of beads and a physical shovel. We performed this sampling activity by hand so that we could develop our intuition about the ideas behind sampling. In this section, we mimic this physical sampling with *virtual* sampling, using a computer. We have a virtual urn and a virtual shovel. 

### Using the virtual shovel once

Virtual sampling requires a virtual urn. Create a tibble named `urn`. The rows of `urn` correspond exactly to the contents of the actual urn. 
...
urn <- tibble(color = c(rep("red", 400), rep("white", 600))) %>%
  ...
  # beads. But we find it aesthetically pleasing.
...
Now that we have a virtual urn, we need a virtual shovel. We use  `slice_sample()` and list-columns. Take a sample of 50 beads from our virtual urn.
...
As usual, map functions and list-columns are powerful but confusing. The `str()` is a good way to explore tibbles with list-columns.
