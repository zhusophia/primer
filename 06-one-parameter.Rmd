...
<!-- MF: References to code chunks not working. I'm troubleshooting now. Create a simple example (three line Rmd file) and then ask on RStudio Community.  -->

<!-- Add to Maslow something like 95% confidence interval squiggly mean plus/minus 2 standard errors -->

<!-- Make the stan_glm graphic look like the Using the posterior graphic. -->


<!-- Find a place to include these useful sentences: -->

<!-- $\hat{p}$ is the estimated value of p which comes from taking a sample. There can be billions and billions of $\hat{p}$'s.  -->


<!-- p -> to $\hat{p}$ to the standard error of $\hat{p}$ to our posterior probability distribution for p. Yellow brick road? Wizard of Posterior? -->

<!-- You must understand what the standard error of $\hat{p}$ means. You do not need to understand why. -->

<!-- Drop Mean of proportions red part of table. -->
...
<!-- Make the last portion of The moral of the story better. Delete old stuff! After the Maslow graphic, you have two paragraphs to make the key point. Use similar stuff in Chapter summary, especially the key sentence. -->
...
<!-- Explain in a comment the reason for  fun = function(.x) 1/(2 * sqrt(.x)) in the plot. I don't know why we need the 2. -->

<!-- Use this quote: "May the odds be ever in your favor!" Or, better, "May the posterior be ever in your favor!" -->

<!-- in Discussion, discuss hypothesis tests and why we hate them. New section. After stan_glm().  Cover most of the points here: https://ppbds.github.io/primer/five-parameters.html#testing. One hypothesis: There are equal number of red and white beads in the urn. Can we reject that hypothesis? Convention: if the 95% confidence interval excludes the null hypthesis, then we reject it. Otherwise, we don't reject it. (Not we accept it.)
...
<!-- Within what range would you offer 50/50 odds that the true percentage lies? Discuss posterior_interval() -->
...
The answer is about `r round(100 *sum(post_dist$new_reds > 8) /  length(post_dist$new_reds))`%. 
