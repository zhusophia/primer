...
<!-- AG: We may want to include a section about how you're supposed to put \ whenever there's a space, since 90% of people will have a space in their file name. However, because we don't have a section about it, we get a lot of confusion about why somebody's thing isn't working. -->
...
Consider the following csv file: `test_2.csv`. We set `file_1` to be the full path to `test_2.csv`'s location on the web.

<!-- AG: This looks really ugly on the actual website because we have parsing failures just out in the open. It looks like a column vs rowwise parsing failure, and it's really confusing if you see this kind of junk. -->
```{r}
...
As you can see, there is text at the top of this file. Often times information about how data was collected, or other relevant information, is included at the top of the data file. This use of `read_csv()` obviously fails to read in the data correctly.
...
<!-- AG: This needs an example, it's weird if we just have the text open like this. -->
...
<!-- AG: Why is all of this code commented out? This passage is incomprehensible without it. -->
...
<!-- AG: I think this database is broken. When I tried to run it, it gave me a Connection timed out error. -->
```
library(RPostgres)
