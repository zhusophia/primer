...
When you click on [data.census.gov](https://data.census.gov)., you will be brought to a homapage with a search bar. You can then type the topic of interest.

```{r}
knitr::include_graphics("maps/images/searchbar.png")
```

For example, you can search "resident population", and it will bring you to a page of various sources. Under each bolded title, you will notice there are different bolded terms. *Survey/Program* tells you which data source is being used. We will only be focusing on the ACS and Decennial Census sources:
...
<!-- BG: Need to figure out, then explain here how we knew the rural population variable  -->

Here, we created a tibble with states in the rows and total population ("P001001") and rural population ("P002005") in the columns. For our next step, we want to plot the proportion of each state's population that lives in rural areas is now a simple application of **tidyverse** functions we know and love. 

First, let's create a variable for rural population proportion and order the states by that variable.
